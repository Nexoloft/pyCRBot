...
class ImageDetector:
    ...
    def find_template(
        ...
        if screenshot is None:
            print(
                f"[{self.instance_name}] No screenshot provided for template matching"
            )
            return None, None
    ...
    def check_pixel_color(
        self, screenshot, x, y, expected_color=(255, 255, 255), tolerance=30
    ):
        ...
        if y >= height or x >= width:
            print(
                f"[{self.instance_name}] Pixel coordinates ({x}, {y}) out of bounds for screenshot size ({width}x{height})"
            )
            return False
        ...
        pixel_rgb = (
            ...
            int(pixel_bgr[1]),
            int(pixel_bgr[0]),
        )  # Convert BGR to RGB as integers
    ...
    def debug_current_screen(self, screenshot, find_template_func):
        ...
        for template_name in templates_to_check:
            ...
            if position:
                print(
                    f"[{self.instance_name}] DEBUG: Found {template_name} at {position} (confidence: {confidence:.2f})"
                )
            else:
                if confidence is not None:
                    print(
                        f"[{self.instance_name}] DEBUG: {template_name} not found (best confidence: {confidence:.2f})"
                    )
                else:
                    print(
                        f"[{self.instance_name}] DEBUG: {template_name} not found (no confidence)"
                    )
