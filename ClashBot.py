...
class EmulatorBot:
    ...
    def play_card(self, check_elixir=False):
        ...
        # Generate a more strategic position within the play area
        # Favor bridge and center positions for better card placement
    ...
    def play_double_card_combo(self):
        ...
        # Generate a single strategic position for both cards
        # Favor bridge positions for combos
            ...
            while self.running and not shutdown_requested:
                ...
                while self.running and not shutdown_requested:
                    ...
                    if time_since_last_play >= min_play_interval:
                        ...
                        if is_2x_elixir:
                            # During 2x elixir: prioritize double combos when we have enough elixir
                            if current_elixir is None:
                                current_elixir = self.detect_elixir_amount()
                                
                            if current_elixir is not None and current_elixir >= 8:
                                if self.play_double_card_combo():
                                    combos_played += 1
                                    print(f"[{self.instance_name}] Successfully played double card combo {combos_played} (2x elixir mode)")
                                    card_played = True
                                    last_card_play = current_time
                                    battle_start_time = current_time  # Reset battle timer on successful play
                            elif current_elixir is not None and current_elixir >= 4:
                                # Play single card if we have some elixir but not enough for combo
                                if self.play_card(check_elixir=True):
                                    single_cards_played += 1
                                    print(f"[{self.instance_name}] Successfully played single card {single_cards_played} (2x elixir fallback)")
                                    card_played = True
                                    last_card_play = current_time
                                    battle_start_time = current_time
                        else:
                            # During normal elixir: play single cards with smart timing
                            target_elixir = 4
                            if battle_phase == "late":
                                target_elixir = 3  # Be more aggressive late in battle
                            ...
                            if current_elixir is not None and current_elixir >= target_elixir:
                                if self.play_card(check_elixir=True):
                                    single_cards_played += 1
                                    print(f"[{self.instance_name}] Successfully played single card {single_cards_played} (normal elixir mode, {battle_phase} phase)")
                                    card_played = True
                                    last_card_play = current_time
                                    battle_start_time = current_time
                        
                        if not card_played:
                            # If we couldn't play a card, wait a bit before trying again
                            time.sleep(0.5)
                    else:
