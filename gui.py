...
import threading
from main import (
    detect_memu_instances,
    verify_template_images,
    run_battle_mode,
    run_upgrade_mode,
    run_battlepass_mode,
)
...
class ClashRoyaleBotGUI:
    ...
    def create_widgets(self):
        ...
        instances_frame = ttk.LabelFrame(
            self.root, text="Available MEmu Instances", padding=10
        )
        instances_frame.pack(fill="x", padx=20, pady=10)

        self.instances_listbox = tk.Listbox(
            instances_frame, height=6, selectmode=tk.EXTENDED
        )
        self.instances_listbox.pack(fill="x")

        refresh_btn = ttk.Button(
            instances_frame, text="Refresh Instances", command=self.refresh_instances
        )
        ...
        # Initial status
        self.log_status(
            "GUI initialized. Click 'Refresh Instances' to detect MEmu emulators."
        )
        ...
        try:
            ...
            if self.instances:
                for device_id, instance_name in self.instances:
                    self.instances_listbox.insert(
                        tk.END, f"{instance_name} ({device_id})"
                    )
    ...
    def start_bot(self):
        ...
        if not selected_indices:
            messagebox.showerror(
                "Error", "No MEmu instances selected. Please select at least one."
            )
            return
        ...
        if not self.instances:
            messagebox.showerror(
                "Error", "No MEmu instances available. Please refresh instances."
            )
            return
        ...
        self.log_status(
            f"🚀 Starting bot in {mode} mode with {len(selected_instances)} instance(s)..."
        )
        ...
        if mode == "battle":
            self.bot_thread = threading.Thread(
                target=self._run_battle_mode, args=(selected_instances,), daemon=True
            )
        elif mode == "upgrade":
            self.bot_thread = threading.Thread(
                target=self._run_upgrade_mode, args=(selected_instances,), daemon=True
            )
        elif mode == "battlepass":
            self.bot_thread = threading.Thread(
                target=self._run_battlepass_mode,
                args=(selected_instances,),
                daemon=True,
            )
