...
class BattleRunner:
    ...
    def run_bot_loop(self):
        ...
        while self.bot.running and not self.shutdown_check():
            try:
                ...
                self.logger.change_status(
                    f"--- Starting round {game_round} (Battle {self.battle_count + 1}) ---"
                )
                ...
                if first_battle:
                    if not self.bot.start_first_battle():
                        ...
                        if time.time() - last_activity_time > INACTIVITY_TIMEOUT:
                            self.logger.log(
                                f"No activity for {INACTIVITY_TIMEOUT} seconds, restarting app..."
                            )
                ...
                if not self.bot.wait_for_battle_start(use_fallback=True):
                    self.logger.log(
                        "Battle didn't start properly, trying to recover..."
                    )
                    ...
                    while (
                        ...
                        self.logger.log(
                            "Looking for battle icon after fallback clicks..."
                        )
                        ...
                        if screenshot is not None:
                            ...
                            if battle_pos:
                                self.logger.log(
                                    f"Found battle icon (confidence: {battle_confidence:.2f}), clicking to start battle..."
                                )
                        ...
                        if recovery_attempts == 2:
                            ...
                            self.bot.tap_screen(
                                21, 511
                            )  # Another alternative fallback position
                            time.sleep(2)

                            # After final fallback clicks, look for battle icon again
                            self.logger.log(
                                "Looking for battle icon after final fallback clicks..."
                            )
                            screenshot = self.bot.take_screenshot()
                            if screenshot is not None:
                                battle_pos, battle_confidence = self.bot.find_template(
                ...
                while battle_loop_attempts < max_battle_loop_attempts:
                    ...
                    self.logger.log(
                        f"Battle loop attempt {battle_loop_attempts}/{max_battle_loop_attempts}"
                    )
                    ...
                    if self._enhanced_fight_loop():
                        ...
                        self.logger.log(
                            f"Battle loop failed (attempt {battle_loop_attempts}/{max_battle_loop_attempts})"
                        )
                        if battle_loop_attempts < max_battle_loop_attempts:
                            ...
                            self.logger.log(
                                "Battle loop failed after all attempts, restarting..."
                            )
                            self.bot.restart_app()
                            last_activity_time = time.time()
                            first_battle = True
                ...
                while battle_end_attempts < MAX_BATTLE_END_ATTEMPTS:
                    ...
                    if self._handle_battle_end():
                        ...
                        if battle_end_attempts < MAX_BATTLE_END_ATTEMPTS:
                            ...
                            self.logger.log(
                                "Failed to handle battle end after all attempts, restarting app..."
                            )
                            self.bot.restart_app()
                            last_activity_time = time.time()
                            first_battle = True
        ...
        while self.bot.running and not self.shutdown_check():
            ...
            if should_play:
                if self.bot.play_card_strategically():
                    ...
                    self.logger.change_status(
                        f"Played card #{cards_played_this_battle} this battle"
                    )
    ...
    def _handle_battle_end(self):
        ...
        while time.time() - start_time < post_battle_timeout:
            ...
            play_again_pos, confidence = self.bot.find_template(
                "play_again", screenshot
            )
            if play_again_pos:
                self.logger.log(
                    f"Found Play Again button (confidence: {confidence:.2f}), clicking..."
                )
            ...
            if ok_pos:
                self.logger.log(
                    f"Found OK button (confidence: {confidence:.2f}), clicking..."
                )
                ...
                self.logger.log(
                    "Searching for battle button after returning to home screen..."
                )
                ...
                while time.time() - battle_search_start < battle_search_timeout:
                    ...
                    if screenshot is not None:
                        ...
                        if battle_pos:
                            self.logger.log(
                                f"Found Battle button (confidence: {battle_confidence:.2f}), clicking for next match..."
                            )
                            self.bot.tap_screen(battle_pos[0], battle_pos[1])
                            return True
                ...
                self.logger.log(
                    "Battle button search timed out, trying fallback position..."
                )
