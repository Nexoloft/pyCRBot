...
def run_upgrade_mode(instances, logger_callback=None):
    ...
    for device_id, instance_name in instances:
        bot = EmulatorBot(
            device_id,
            instance_name,
            use_console_display=logger_callback is None,
    ...
    for i, instance in enumerate(instances):
        ...
        if isinstance(instance, dict):
            ...
            bot = EmulatorBot(
                device_id,
                instance_name,
                use_console_display=not no_gui and logger_callback is None,
                logger_callback=logger_callback,
            )
        else:
...
def run_battle_mode(instances, max_battles=0, no_gui=False, logger_callback=None):
    ...
    with ThreadPoolExecutor(max_workers=len(battle_runners)) as executor:
        try:
            # Submit bot tasks
            futures = [
                executor.submit(runner.run_bot_loop) for runner in battle_runners
            ]
...
def main(mode="battle", **kwargs):
    ...
    if mode == "single":
        ...
        instances = [
            {"device_id": device_id, "instance_name": instance_name, "port": port}
        ]
