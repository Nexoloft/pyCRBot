...
class EmulatorBot:
    ...
    def __init__(
        self, device_id, instance_name, use_console_display=True, logger_callback=None
    ):
        ...
        self.logger = Logger(
            instance_name,
            timed=True,
            use_console_display=use_console_display,
        ...
        if success:
            ...
            while time.time() - restart_time < 30:  # Max 30 seconds wait after restart
                ...
                if time.time() - restart_time >= 10 and not fallback_triggered:
                    self.logger.log(
                        "No battle detected 10 seconds after restart, trying fallback clicks..."
                    )
    ...
    def find_template(
        self, template_name, screenshot=None, confidence=CONFIDENCE_THRESHOLD
    ):
    ...
    def wait_for_battle_start(self, use_fallback=True, timeout=None):
        ...
        while time.time() - start_time < timeout:
            ...
            if use_fallback and elapsed >= 10 and not fallback_triggered:
                self.logger.log(
                    "No battle detected after 10 seconds, trying fallback clicks..."
                )
                if self.fallback_click_sequence():
                    return True  # Battle found after fallback
                fallback_triggered = True
            ...
            if (
                use_fallback and elapsed >= 5
            ):  # After 5 seconds, start clicking deadspace
                if random.randint(0, 2) == 0:  # 33% chance each cycle
                    deadspace_position = FALLBACK_POSITIONS.get("deadspace", (21, 511))
                    self.tap_screen(
                        deadspace_position[0], deadspace_position[1]
                    )  # Deadspace click to handle UI

            self.logger.change_status(
                f"Battle not detected yet, waiting... ({elapsed:.1f}s)"
            )
            time.sleep(1)
    ...
    def fallback_click_sequence(self):
        ...
        for i in range(FALLBACK_CLICK_COUNT):
            ...
            if i < FALLBACK_CLICK_COUNT - 1:  # Don't wait after the last click
                ...
                if self.is_in_battle():
                    self.logger.log(
                        f"✓ Battle detected after fallback click {i+1} wait!"
                    )
                    return True
        ...
        while (
            time.time() - fallback_check_start < 20
    ...
    def start_first_battle(self):
        ...
        while time.time() - start_time < 30:
            ...
            battle_position, confidence = self.find_template(
                "battle_button", screenshot
            )
            if battle_position:
                self.logger.log(
                    f"Found Battle button (confidence: {confidence:.2f}), clicking to start battle..."
                )
    ...
    def wait_for_elixir_strategic(self, target_elixir=None):
        ...
        while time.time() - start_time < timeout:
            ...
            if current_elixir is not None and current_elixir >= target_elixir:
                self.logger.log(
                    f"Have {current_elixir} elixir (needed {target_elixir})"
                )
                return True
    ...
    def auto_upgrade_cards(self):
        ...
        while self.running:
            ...
            upgrade_possible_pos, confidence = self.find_template(
                "upgrade_possible", screenshot
            )

            if upgrade_possible_pos:
                upgrade_count += 1
                self.logger.log(
                    f"Upgrade #{upgrade_count}: Found upgrade_possible (confidence: {confidence:.2f})"
                )

                # Click upgrade_possible
                if not self.tap_screen(
                    upgrade_possible_pos[0], upgrade_possible_pos[1]
                ):
                ...
                screenshot = self.take_screenshot()
                upgrade_button_pos, button_confidence = self.find_template(
                    "upgrade_button", screenshot
                )
                if upgrade_button_pos:
                    self.logger.log(
                        f"Found upgrade_button (confidence: {button_confidence:.2f}), clicking..."
                    )
                    ...
                    screenshot = self.take_screenshot()
                    upgrade_button_pos2, button_confidence2 = self.find_template(
                        "upgrade_button", screenshot
                    )
                    if upgrade_button_pos2:
                        self.logger.log(
                            f"Found upgrade_button again (confidence: {button_confidence2:.2f}), clicking..."
                        )
                ...
                screenshot = self.take_screenshot()
                confirm_pos, confirm_confidence = self.find_template(
                    "confirm", screenshot
                )
                if confirm_pos:
                    self.logger.log(
                        f"Found Confirm button (confidence: {confirm_confidence:.2f}), clicking..."
                    )
                ...
                while click_attempts < max_click_attempts and self.running:
                    ...
                    if screenshot is not None:
                        next_upgrade_pos, next_confidence = self.find_template(
                            "upgrade_possible", screenshot
                        )
                        if next_upgrade_pos:
                            self.logger.log(
                                f"Found next upgrade after {click_attempts} clicks"
                            )
                            break
        ...
        self.logger.log(
            f"Auto upgrade sequence finished. Total upgrades: {upgrade_count}"
        )
    ...
    def auto_claim_battlepass(self):
        ...
        if not os.path.exists(template_path):
            self.logger.log(
                f"ERROR: ClaimRewards template not found at {template_path}"
            )
            return 0
        else:
        ...
        try:
            while self.running:
                ...
                while (
                    click_attempts < max_click_attempts
                    and self.running
                    and not found_claim_button
                ):
                    try:
                        ...
                        if screenshot is not None:
                            try:
                                ...
                                if claim_rewards_pos:
                                    claims_count += 1
                                    self.logger.log(
                                        f"Claim #{claims_count}: Found ClaimRewards button (confidence: {confidence:.2f}) after {click_attempts} clicks"
                                    )

                                    # Click the ClaimRewards button
                                    if self.tap_screen(
                                        claim_rewards_pos[0], claim_rewards_pos[1]
                                    ):
                                        self.logger.log(
                                            "Successfully clicked ClaimRewards button"
                                        )
                                ...
                                if (
                                    click_attempts % 20 == 0
                                ):  # Log template errors less frequently
                                    self.logger.log(
                                        f"Template detection error on attempt {click_attempts}: {template_error}"
                                    )
                        else:
                            if (
                                click_attempts % 20 == 0
                            ):  # Log screenshot failures less frequently
                                self.logger.log(
                                    f"Failed to take screenshot on attempt {click_attempts}"
                                )

                    except Exception as e:
                        self.logger.log(
                            f"Error during click attempt {click_attempts}: {e}"
                        )
                        continue
                ...
                if found_claim_button:
                    ...
                    while time.time() - start_time < timeout_duration and self.running:
                        try:
                            ...
                            if screenshot is not None:
                                try:
                                    next_claim_pos, next_confidence = (
                                        self.find_template("ClaimRewards", screenshot)
                                    )
                                    if next_claim_pos:
                                        self.logger.log(
                                            f"Found next ClaimRewards button (confidence: {next_confidence:.2f}) after {time.time() - start_time:.1f} seconds"
                                        )
                    ...
                    if not found_next_claim:
                        self.logger.log(
                            "No ClaimRewards button found after 15 seconds timeout"
                        )
                    ...
                    self.logger.log(
                        "Reached maximum click attempts at (114, 271) without finding ClaimRewards"
                    )
                    self.logger.log(
                        "No more battlepass rewards available or unable to progress further"
                    )
                    break
        ...
        self.logger.log(
            f"Auto battlepass claiming sequence finished. Total claims: {claims_count}"
        )
        return claims_count
